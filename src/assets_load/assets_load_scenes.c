...
int assets_load_scenes(game_scene_t *scenes)
{
    for (int tmp = MAIN_MENU; tmp <= DEFEAT; tmp++) {
        if (assets_load_objs(scenes[tmp].objs, scenes[tmp].how_many[0]) == 84)
            return 84;
        if (assets_load_sounds(scenes[tmp].sounds, scenes[tmp].how_many[1]) == 84)
            return 84;
        if (assets_load_buttons(scenes[tmp].buttons, scenes[tmp].how_many[2]) == 84)
            return 84;
        if (assets_load_texts(scenes[tmp].texts, scenes[tmp].how_many[3]) == 84)
            return 84;
    }
    return 0;
}