...
void enemies_positions(game_t *game, int tmp, float delta_time)
{
    ...
    if (game->stats->player.position.y < vector.y)
        vector.y -= game->scenes[game->settings->current].objs[tmp].speed * delta_time;
    if (game->stats->player.position.y > vector.y)
        vector.y += game->scenes[game->settings->current].objs[tmp].speed * delta_time;
    if (game->stats->player.position.x < vector.x)
        vector.x -= game->scenes[game->settings->current].objs[tmp].speed * delta_time;
    if (game->stats->player.position.x > vector.x)
        vector.x += game->scenes[game->settings->current].objs[tmp].speed * delta_time;
    sfSprite_setPosition(game->scenes[game->settings->current].objs[tmp].sprite, vector);
