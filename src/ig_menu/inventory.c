...
void open_inventory(game_setting_t *settings, game_scene_t *scene)
{
    if (settings->event.key.code == sfKeyE)
        sfRenderWindow_drawSprite(settings->window,
            scene->objs[INVENTORY_O_S1].sprite, NULL);
}
...
void draw_cursor(game_setting_t *settings, game_scene_t *scenes)
{
    if (settings->current >= main_menu && settings->current <= defeat)
        sfSprite_setPosition(scenes->objs[CURSOR_O_S0].sprite,
            settings->cursor_pos);
}