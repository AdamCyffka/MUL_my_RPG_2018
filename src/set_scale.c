...
void set_scale4(game_t *game)
{
    if (game->settings->current == FOREST) {
        ...
        for (int tmp = GHOST1_O_S3; tmp <= GHOST15_O_S3; tmp++) {
            ...
            (sfVector2f) {3.5, 3.5});
            sfSprite_setScale(game->scenes[FOREST].objs[MINIMAP_O_S3].sprite,
            (sfVector2f) {0.8, 0.8});
        }
    }
}

void set_scale3(game_t *game)
{
    if (game->settings->current == TOWN)
        sfSprite_setScale(game->scenes[TOWN].objs[MINIMAP_O_S1].sprite,
        (sfVector2f) {0.8, 0.8});
    if (game->settings->current == BOSS) {
        ...
        (sfVector2f) {7, 7});
        sfSprite_setScale(game->scenes[BOSS].objs[MINIMAP_O_S2].sprite,
        (sfVector2f) {0.8, 0.8});
    }
    set_scale4(game);
}
...
void set_scale2(game_t *game)
{
    if (game->settings->current == BEACH) {
        for (int tmp = SKELETON1_O_S4; tmp <= SKELETON20_O_S4; tmp++) {
            ...
            (sfVector2f) {3.5, 3.5});
            sfSprite_setScale(game->scenes[BEACH].objs[MINIMAP_O_S4].sprite,
            (sfVector2f) {0.8, 0.8});
        }
    }
    if (game->settings->current == CAMP) {
        for (int tmp = GOLEM1_O_S5; tmp <= GOLEM10_O_S5; tmp++) {
            ...
            (sfVector2f) {3.5, 3.5});
            sfSprite_setScale(game->scenes[CAMP].objs[MINIMAP_O_S5].sprite,
        (sfVector2f) {0.8, 0.8});
        }
    }
    set_scale3(game);
